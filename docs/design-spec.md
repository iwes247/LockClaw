...
- **Hardened SSH profile**: disable root and password auth; keep key-based admin access only.
- **Firewall enabled by default**: deny inbound by default and only open explicitly approved ports.
- **Kernel/account/sudo hardening overlays**: raises baseline resistance against common local escalation paths.
- **Audit + persistent logging**: ensures incident traceability.
- **Fixed update posture**: default to pinned OpenClaw tag/version and explicit update steps (`openclaw doctor` after update).
...
- **DNS**: explicit resolver configuration with DNSSEC allowance and optional DoT mode.
- **DHCP**: default IPv4 addressing via DHCP on active uplink.
- **Time sync**: explicit NTP servers to keep TLS/verification and logs reliable.
...
- SSH (admin plane): `22/tcp` (enabled with hardened config)
